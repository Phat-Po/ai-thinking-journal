...
> [!summary] 今天
> 2-3 sentences: what occupied the day, the most important decision, one thing learned.

### 项目时间线

For each meaningful project (skip trivial one-off queries), write a short paragraph:
- Start with the project name and which tool was used: "(via Claude Code)" or "(via Codex)"
...
<example>
> [!summary] 今天
> 主力在搞日记自动化 pipeline，从提取到生成跑通了全流程。顺便把 VIBE Dashboard 46 个乱 commit squash 成 5 个。学到：prompt 没写清楚语气，输出就变公文。
